...
class AdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.location = Location.objects.create(
            id="1", title="Test Location", country_code="US", location_type="Country", center=Point(-74.0060, 40.7128)
        )
    ...
    def test_admin_can_create_property(self):
        ...
        response = self.client.post(reverse('admin:inventory_accommodation_add'), {
            'title': 'Admin Property',
            'country_code': 'US',
            'bedroom_count': 2,
            'review_score': 4.5,
        ...
        self.assertRedirects(response, reverse('admin:inventory_accommodation_changelist'))
    ...
    def test_admin_can_update_property(self):
        ...
        accommodation = Accommodation.objects.create(
            id="1", title="Old Property", country_code="US", bedroom_count=2, review_score=3.0,
            usd_rate=100.00, center="POINT(40.7128 -74.0060)", location=self.location, user=self.admin_user, published=True
        )

        # Access the property change page
        response = self.client.get(reverse('admin:inventory_accommodation_change', args=[accommodation.id]))
        self.assertEqual(response.status_code, 200)

        # Update property data
        response = self.client.post(reverse('admin:inventory_accommodation_change', args=[accommodation.id]), {
    ...
    def test_admin_can_delete_property(self):
        ...
        # Create a property
        accommodation = Accommodation.objects.create(
            id="1", title="Property to Delete", country_code="US", bedroom_count=2, review_score=4.5,
            usd_rate=150.00, center="POINT(40.7128 -74.0060)", location=self.location, user=self.admin_user, published=True
        )
        ...
        response = self.client.post(reverse('admin:inventory_accommodation_delete', args=[accommodation.id]))
    ...
    def test_import_csv_view(self):
        ...
        response = self.client.post(reverse('admin:location_import_csv'), {'csv_file': csv_file})
    ...
    def test_custom_admin_permissions_for_property_owner(self):
        ...
        accommodation = Accommodation.objects.create(
            id="1", title="Owner's Property", country_code="US", bedroom_count=2, review_score=4.5,
            usd_rate=150.00, center="POINT(40.7128 -74.0060)", location=self.location, user=self.property_owner_user, published=True
        )
        ...
        self.assertNotContains(response, "Admin Property")  # Admin Property should not be visible to the property owner
    ...
    def test_admin_can_view_and_edit_locations(self):
        ...
        # Create a location
        location = Location.objects.create(
            id="1", title="Test Location", country_code="US", location_type="Country", center="POINT(40.7128 -74.0060)"
        )

        # Ensure admin can view the location
        response = self.client.get(reverse('admin:inventory_location_change', args=[location.id]))
        self.assertEqual(response.status_code, 200)

        # Update location
        response = self.client.post(reverse('admin:inventory_location_change', args=[location.id]), {
