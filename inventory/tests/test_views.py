...
class ViewsTest(TestCase):
    ...
    def test_property_owner_add_property(self):
        ...
        self.assertRedirects(response, reverse('property_list'))  # After creation, redirect to property list
        self.assertEqual(Accommodation.objects.count(), 1)  # Ensure property was created
    ...
    def test_property_owner_update_property(self):
        ...
        accommodation = Accommodation.objects.create(
            ...
            usd_rate=100.00, center="POINT(40.7128 -74.0060)", location=self.location, user=self.property_owner_user, published=True
        )

        # Update property data
        response = self.client.post(reverse('update_property', args=[accommodation.id]), {
        ...
        self.assertRedirects(response, reverse('property_list'))  # After updating, redirect to property list
    ...
    def test_property_owner_delete_property(self):
        ...
        self.assertRedirects(response, reverse('property_list'))  # Redirect after deletion
    ...
    def test_property_list_for_logged_in_user(self):
        ...
        accommodation = Accommodation.objects.create(
            id="1", title="User's Property", country_code="US", bedroom_count=2, review_score=4.0,
            usd_rate=100.00, center="POINT(40.7128 -74.0060)", location=self.location, user=self.property_owner_user, published=True
        )

        response = self.client.get(reverse('property_list'))
        self.assertContains(response, "User's Property")  # Ensure the property is listed
